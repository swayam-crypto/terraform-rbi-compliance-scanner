...
from compliance_scanner.core import scan_directory_large
...
def test_cache_produces_identical_findings_on_second_run(tmp_path):
    ...
    first_run = list(
        scan_directory_large(str(tmp_path), use_cache=True, cache_path=cache_path)
    )
    second_run = list(
        scan_directory_large(str(tmp_path), use_cache=True, cache_path=cache_path)
    )
...
def test_cache_invalidates_on_file_change(tmp_path):
    ...
    assert (
        "RBI-001" not in first_run_rule_ids
    )  # region is compliant, no localization violation
    ...
    assert (
        "RBI-001" in second_run_rule_ids
    )  # cache correctly detected the change and re-scanned
