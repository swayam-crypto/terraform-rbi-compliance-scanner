...
class TerraformParser:
    def parse_directory(self, path: Path) -> list[ResolvedResource]:
        ...
        return [ResolvedResource(platform=Platform.TERRAFORM, provider=infer_provider(resource_type), resource_type=resource_type, resource_name=resource_name, attributes=config, default_attributes=_resolve_provider_for_resource(resource_type, providers), source=SourceLocation(file_path=file_path)) for file_path, resources in files.items() for resource_type, named in resources.items() for resource_name, config in named.items()]
