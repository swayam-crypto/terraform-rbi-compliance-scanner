...
def scan_directory_large(
    ...
    if files_to_parse:
        ...
        # iter_terraform_directory expects a directory, so if we're
        # only re-parsing a subset, we parse them individually here
        # in parallel using the same underlying parse function.
